...
export default function NewList() {
  ...
  return (
    <div className=" mx-auto  md:p-4"> {/* Reduzido de p-4 para p-2 */}
  <div className="bg-white rounded-xl shadow-md overflow-hidden">
    {/* Cabeçalho */}
    <div className="bg-gray-900 text-white px-3 py-2 md:px-4 md:py-3 flex justify-between items-center">
    ...
    <div className="p-2 space-y-4 md:hidden"> {/* Reduzido de p-4 para p-2 */}
      {Lancamentos.length > 0 ? (
        Lancamentos.map((lan) => {
          ...
          return (
            <div
              key={lan.id}
              className={`rounded-lg p-3 shadow-md ${ // Reduzido de p-4 para p-3
    ...
    <div className="hidden md:block overflow-x-auto">
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> {/* Reduzido de px-6 para px-4 e py-3 para py-2 */}
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {Lancamentos.length === 0 ? (
            <tr>
              <td colSpan="5" className="px-4 py-4 text-center"> {/* Reduzido de px-6 para px-4 e py-4 */}
            ...
            Lancamentos.map((lan) => {
              const isReceita = lan.tipo === "C";
              return (
                <tr key={lan.id} className={`${isReceita ? "bg-green-50" : "bg-red-50"} hover:bg-gray-50`}>
                  <td className="px-4 py-2 whitespace-nowrap"> {/* Reduzido de px-6 para px-4 e py-4 para py-2 */}
                    <div className="flex items-center">
                      <div className="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center">
                        <i className={`fas ${isReceita ? "fa-arrow-down text-green-500" : "fa-arrow-up text-red-500"}`}></i>
                      </div>
                      <div className="ml-4">
                        <div className="text-sm font-medium text-gray-900">{lan.descricao}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{typeof lan.categoria === "string" ? lan.categoria : "Categoria"}</td>
                  <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{lan.data?.toDate().toLocaleDateString('pt-BR')}</td>
                  <td className="px-4 py-2 whitespace-nowrap text-sm font-medium">
                    <span className={`${isReceita ? "text-green-600" : "text-red-600"}`}>R$ {parseFloat(lan.valor).toFixed(2).replace('.', ',')}</span>
                  </td>
                  <td className="px-4 py-2 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${isReceita ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
                      {isReceita ? "Receita" : "Despesa"}
                    </span>
                  </td>
                </tr>
